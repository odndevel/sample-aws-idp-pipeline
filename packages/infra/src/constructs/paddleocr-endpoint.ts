...
export interface PaddleOcrEndpointProps {
  ...
  maxCapacity?: number;
  /**
   * Scale-in cooldown in minutes (time to wait before scaling down)
   * @default 15
   */
...
export class PaddleOcrEndpoint extends Construct {
  ...
  constructor(scope: Construct, id: string, props: PaddleOcrEndpointProps) {
    ...
    // Auto-scaling configuration (default enabled with min=1)
    const enableAutoScaling = props.enableAutoScaling ?? true;
    if (enableAutoScaling) {
      const minCapacity = props.minCapacity ?? 1;
      const maxCapacity = props.maxCapacity ?? 1;
      const scaleInCooldownMinutes = props.scaleInCooldownMinutes ?? 15;
      ...
      // Scale out when there are pending requests (HasBacklogWithoutCapacity)
      // This metric is 1 when there are requests but no capacity, 0 otherwise
      ...
      // Scale in when idle (ApproximateBacklogSizePerInstance = 0)
      const backlogSizeMetric = new cloudwatch.Metric({
        namespace: 'AWS/SageMaker',
      ...
      // Target tracking to scale in when no backlog
      scalableTarget.scaleToTrackMetric('ScaleToZeroWhenIdle', {
        targetValue: 1, // Target 1 request per instance
        customMetric: backlogSizeMetric,
        scaleInCooldown: Duration.minutes(scaleInCooldownMinutes),
