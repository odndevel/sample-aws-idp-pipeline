...
LANCEDB_FUNCTION_NAME = os.environ.get(
    "LANCEDB_FUNCTION_NAME", "idp-v2-lancedb-service"
)
...
def handler(event, _context):
    ...
    if not content_combined.strip():
        print(
            f"Error: content_combined is empty for segment {segment_index}. Aborting."
        )
        update_segment_status(
            file_uri, segment_index, SegmentStatus.FAILED, error="No content to index"
        )
        raise Exception(f"No content to index for segment {segment_index}")
    ...
    try:
        ...
        response = client.send_message(
            QueueUrl=LANCEDB_WRITE_QUEUE_URL, MessageBody=json.dumps(message)
        )

        print(
            f'Sent segment {segment_index} to SQS, MessageId: {response["MessageId"]}'
        )
        ...
        update_segment_status(
            file_uri, segment_index, SegmentStatus.FAILED, error=str(e)
        )
        return {
            "workflow_id": workflow_id,
            "segment_index": segment_index,
