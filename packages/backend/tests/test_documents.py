...
class TestGetWorkflow:
    @patch("app.ddb.workflows.get_table")
    def test_get_workflow_success(self, mock_get_table):
        mock_table = MagicMock()
        mock_table.get_item.return_value = {
            "Item": {
                "PK": "WF#wf-1",
            ...
            "Items": [
                {
                    ...
                    "data": {
                        "segment_index": 0,
                        "image_uri": "",
                        "bda_indexer": "BDA content",
                        "format_parser": "Parsed content",
                        "image_analysis": [],
                    },
                },
            ]
        ...
        response = client.get("/projects/proj-1/workflows/wf-1")
    ...
    @patch("app.ddb.workflows.get_table")
    def test_get_workflow_not_found(self, mock_get_table):
        mock_table = MagicMock()
        mock_table.get_item.return_value = {}
        mock_get_table.return_value = mock_table

        response = client.get("/projects/proj-1/workflows/nonexistent")

        assert response.status_code == 404
        assert response.json()["detail"] == "Workflow not found"

    @patch("app.ddb.workflows.get_table")
    def test_get_workflow_wrong_project(self, mock_get_table):
        mock_table = MagicMock()
        mock_table.get_item.return_value = {
            "Item": {
                "PK": "WF#wf-1",
        ...
        assert response.status_code == 404
        assert response.json()["detail"] == "Workflow not found in this project"
